...
public class LogisticMemoService {
    ...
    public LogisticMemoResponse newMemo(LogisticMemoRequest request, String userId) throws IOException {
        ...
        if (request.getMemo_upload() != null) {
            logisticMemo.setMemo_upload(Base64.getEncoder().encodeToString(request.getMemo_upload().getBytes()));
        }
    ...
    @Transactional
    public LogisticMemoResponse editMemo(String memoId, LogisticMemoRequest request, String userId) throws IOException {
        validationService.validateRequest(request);
